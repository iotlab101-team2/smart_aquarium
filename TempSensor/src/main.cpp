...
void loop() {
    ...
    if(currentMillis - lastReadMillis >= interval) {
      ...
      client.publish(pub_topic[1].c_str(),"123");
      sprintf(buf,"%d",val); // 밝기 문자열로 변환
      client.publish(pub_topic[2].c_str(),"123");
